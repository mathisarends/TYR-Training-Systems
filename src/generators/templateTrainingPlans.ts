...
import TrainingBlock from "../models/trainingBlock.model.js";

function createTemplatePlanA(userID: ObjectId) {
  const curDate = new Date();
  const trainingWeeks = createExerciseWeeksAndDaysTemplateA();

  const templateTrainingPlan = new TrainingBlock({
    title: "Template A",
    trainingFrequency: 4,
    trainingPhase: "hypertrophie",
    lastUpdated: curDate,
    lastWeekDeloadEnabled: true,
    trainingWeeks: trainingWeeks,
    weightRecommandationBase: "max",
  });
  return templateTrainingPlan;
}

function createTemplatePlanB(userID: ObjectId) {
  const curDate = new Date();
